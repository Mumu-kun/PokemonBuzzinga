import React, { createRef, useState, useEffect, useCallback, useMemo, useRef } from "react";
...
export default function Corregidor({
	...
	const circleStrokeWidth = useMemo(() => (prevSize * CIRCLE_STROKE_WIDTH) / 100, [prevSize]);
	const lineStrokeWidth = useMemo(() => (prevSize * LINE_STROKE_WIDTH) / 100, [prevSize]);

	const animateCircle = useCallback(() => {
		const timeline = gsap.timeline();

		timeline
			.fromTo(circleRef.current, 0.7, { scale: 0, transformOrigin: "center" }, { scale: 1, ease: Power4.easeOut })
			.fromTo(
				circleRef.current,
				0.4,
			...
			.fromTo(circleRef.current, 0.3, { opacity: 1 }, { opacity: 0, ease: Power4.easeOut }, "-=0.2");
	...
	const animateLines = useCallback(() => {
		...
		linesRefs.current.forEach((ref, i) => {
			...
			timeline
				.fromTo(ref.current, 1, { attr: { x2: center, y2: center } }, { attr: start, ease: Power4.easeOut })
				.fromTo(ref.current, 1, { attr: { x1: center, y1: center } }, { attr: end, ease: Power4.easeOut }, "-=0.9");
	...
	}, [prevRepeat, prevDelay, prevRepeatDelay, onComplete, onStart, onRepeat, animateCircle, animateLines]);
