...
app.get("/api/trainer/:trainerId", async (req, res) => {
	try {
		...
		const { rows } = await pool.query(
			...
			(SELECT COUNT(distinct p.pokemon_id) FROM OWNED_POKEMONS op join pokemons p on op.pokemon_id = p.pokemon_id WHERE TRAINER_ID = TR.ID) AS POKEDEX_COUNT,
			(SELECT COUNT(distinct p.pokemon_id) FROM pokemons p) as TOTAL_POKEDEX_COUNT,
