...
app.get("/api/pokemons/:id", async (req, res) => {
  try {
    ...
    const { name, hp, attack, defense, speed, sp_attack, sp_defense, total } =
      rows[0];
...
app.get("/api/pokemons-dets/:id", async (req, res) => {
  try {
    ...
    if (abilityRows.length < 1 || abilityRows.length > 2) {
      throw new Error(
        `Invalid number of abilities for Pokemon with ID:${pokemon_id}.`
      );
    }

    const abilities = abilityRows.map(
      ({ ability_id, ability, description }) => ({
        ability_id,
        ability,
        description,
      })
